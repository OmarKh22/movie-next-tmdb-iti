...
import MovieCard from "../../components/MovieCard"; // تأكد المسار صح بالنسبة لمكان الملف
...
export default function MovieDetailClient({ id }) {
  ...
  useEffect(() => {
    ...
    const fetchMovie = async () => {
      try {
        const res = await axios.get(
          `/api/movies?endpoint=/movie/${id}&append_to_response=production_companies`
        );
        setMovie(res.data);

        // Check if in wishlist
        const wishlist = JSON.parse(localStorage.getItem("wishlist") || "[]");
        const exists = wishlist.some((m) => m.id === res.data.id);
    ...
    const fetchRecommendations = async () => {
      try {
        const res = await axios.get(
          `/api/movies?endpoint=/movie/${id}/recommendations`
        );
        ...
        <div className="flex-1 flex flex-col gap-4 items-start justify-start">
          ...
          <div className="flex flex-col gap-1 mb-2 w-full">
            <span className="text-gray-800 text-sm">
              Duration: {movie.runtime || "Unknown"} min
            </span>
            <span className="text-gray-800 text-sm">
